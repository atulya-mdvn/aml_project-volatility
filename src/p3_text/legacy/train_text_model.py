...
P3_FEATURES_PATH = Path("data/processed/p3_features.parquet")
TARGETS_PATH = Path("data/processed/p5_features.parquet")

PREDICTIONS_OUTPUT_PATH = Path("outputs/p3/predictions.parquet")
METRICS_OUTPUT_PATH = Path("outputs/p3/metrics.json")
...
def load_p3_features(path: Path = P3_FEATURES_PATH) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"P3 feature file not found: {path}")

    df = pd.read_parquet(path)
    if "date" not in df.columns:
        raise ValueError("P3 feature table must contain a 'date' column.")
...
def load_targets(path: Path = TARGETS_PATH) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            ...
            "You need data/processed/p5_features.parquet before training P3 predictions."
        )
...
def get_p3_feature_columns(df: pd.DataFrame) -> list[str]:
...
def prepare_modeling_table(
    ...
    feature_cols = get_p3_feature_columns(p3_df)
    if not feature_cols:
        raise ValueError("No P3 feature columns found for modeling.")

    if merged.empty:
        raise ValueError("Merged modeling table is empty after joining P3 features to targets.")
...
def main() -> None:
    p3_df = load_p3_features(P3_FEATURES_PATH)
    ...
    print("Standalone P3 prediction run complete.")
    print(f"Used {len(feature_cols)} P3 features.")
